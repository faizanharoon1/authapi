...
namespace DAL
{
    public interface IDefaultSQLPolicy
    {
        AsyncRetryPolicy GetAsyncPolicy();
    }
    public class DefaultSQLPolicy : IDefaultSQLPolicy
    {
        private readonly ILogger<DefaultSQLPolicy> _logger;
        public DefaultSQLPolicy(ILogger<DefaultSQLPolicy> loggerFactory)
        {
            _logger = loggerFactory;
        }
        public AsyncRetryPolicy GetAsyncPolicy()
        {
            ...
            (exception, timeSpan, retryCount, context) =>
            {
                _logger.LogWarning(
                   exception,
                   "WARNING: Error talking to Db, will retry after {RetryTimeSpan}. Retry attempt {RetryCount}",
                   timeSpan,
                   retryCount
               );
            });
